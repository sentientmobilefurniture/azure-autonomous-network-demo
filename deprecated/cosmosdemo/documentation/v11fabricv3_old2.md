...
| 22 | **`telco-noc-fabric/` scenario directory missing** — architecture docs mark it "Done" but it was never created. Provisioning pipeline can't find CSVs to upload. `fabric_provision.py` accepts `scenario_name` but ignores it. Reference scripts use hardcoded flat paths that don't match project structure. | Added Phase B0: create scenario data pack (symlinked) + data path resolver function. Tagged as Phase B prerequisite. |
| 23 | **Data Agent references in F7/F8 are wrong** — the architecture does NOT use Fabric Data Agents. Agents send GQL/KQL queries directly to Fabric REST APIs. Data Agent prompts are deprecated in `fabric_implementation_references/data/prompts/deprecated/`. | Removed F7 (Data Agent discovery) and F8 (Data Agent UI). Clarified the direct query architecture throughout. |
---
...
### Data flow architecture (no Data Agents)
...
```
  ...
  │   (composes GQL from                     POST /GraphModels/ │
  │    language_gql.md)                      {id}/executeQuery  │
  │                                                             │
  │  TelemetryAgent ──→ /query/telemetry ──→ FabricKQLBackend   │
  │   (composes KQL from                     KustoClient SDK    │
  │    KQL instructions)                     → Eventhouse       │
  └─────────────────────────────────────────────────────────────┘
```

> **No Fabric Data Agents are used.** The AI agents (GraphExplorerAgent,
> TelemetryAgent) compose GQL/KQL from their language prompts and submit
> queries through the existing backend dispatch layer. `FabricGQLBackend`
> calls the Graph Model REST API directly; `FabricKQLBackend` (Phase F)
> calls the Eventhouse via `azure-kusto-data` SDK directly.
> Data Agent prompts in `fabric_implementation_references/data/prompts/deprecated/`
> are historical artifacts from an earlier design.
...
### B0: Prerequisite — Fabric scenario data pack + data path resolution

> **⚠ CRITICAL GAP:** The provisioning pipeline can't upload data because:
> 1. `telco-noc-fabric/` scenario directory doesn't exist (marked "Done" in
>    architecture docs but never created)
> 2. `fabric_provision.py` accepts `scenario_name` in request models but
>    **never uses it** — no data path resolution, no CSV reading
> 3. The reference scripts use hardcoded flat paths (`data/lakehouse/`,
>    `data/eventhouse/`) that don't match the project's
>    `data/scenarios/{name}/data/` structure

**B0a: Create `data/scenarios/telco-noc-fabric/` scenario pack**

The Fabric scenario **reuses `telco-noc`'s raw data** (same CSVs) but with
different connector config. Create by symlink or copy:

```
data/scenarios/telco-noc-fabric/
├── scenario.yaml          # fabric-gql graph + cosmosdb-nosql telemetry
├── data/
│   ├── entities/          # ← symlink → ../telco-noc/data/entities/
│   ├── telemetry/         # ← symlink → ../telco-noc/data/telemetry/
│   └── prompts/           # ← symlink → ../telco-noc/data/prompts/
└── data/knowledge/
    ├── runbooks/           # ← symlink → ../telco-noc/data/knowledge/runbooks/
    └── tickets/            # ← symlink → ../telco-noc/data/knowledge/tickets/
```

`scenario.yaml` key changes from `telco-noc`:
...
data_sources:
  graph:
    connector: "fabric-gql"                    # ← was "cosmosdb-gremlin"
    config:
  ...
  telemetry:
    connector: "cosmosdb-nosql"                # ← same as telco-noc
    config:
      database: "telemetry"
      container_prefix: "telco-noc-fabric"     # ← own prefix

  # search_indexes: same as telco-noc

agents:
  - name: "GraphExplorerAgent"
    # ...
    instructions_file: "prompts/graph_explorer/"  # same prompts
    compose_with_connector: true                   # ← selects language_gql.md
    # ... rest identical to telco-noc
```

> **Design decision — symlinks vs copies:** Symlinks are cleaner (one source
> of truth) but may break in Docker COPY or tarball packaging. If symlinks
> cause problems, use a build script that copies at deploy time. For now,
> symlinks are the right default.

**B0b: Data path resolution in `fabric_provision.py`**

Add a helper that resolves `scenario_name` → data directories:

```python
from pathlib import Path

# Relative to api/ service root (mounted in container)
...
def _resolve_scenario_data(scenario_name: str) -> dict:
    """Resolve scenario_name to data directory paths.

    Returns dict with keys: entities_dir, telemetry_dir, scenario_yaml.
    Raises ValueError if scenario directory or required paths don't exist.
    """
    scenario_dir = SCENARIOS_DIR / scenario_name
    if not scenario_dir.exists():
        raise ValueError(f"Scenario directory not found: {scenario_dir}")

    # Read scenario.yaml to get relative paths
    import yaml
    ...
    if not entities_dir.exists():
        raise ValueError(f"Entities directory not found: {entities_dir}")
...
B1-B5 below use this resolver. The `scenario_name` field in `FabricProvisionRequest`
(currently ignored) becomes the key input for data path resolution.

> **Why per-scenario paths matter for the flow:** When the user clicks
> "Set Up Resources" in the FabricSetupWizard, the pipeline needs to know
> *which* CSVs to upload. Different scenarios will have different entity
> types, different telemetry tables, and different ontology definitions.
> Hardcoding `telco-noc` paths would make the pipeline single-scenario.
...
Queries Fabric Eventhouse KQL databases via the `azure-kusto-data` SDK.
Enables `telemetry.connector: "fabric-kql"` in scenario.yaml.

> **Architecture note — direct queries, no Data Agents:** The Fabric
> integration does NOT use Fabric Data Agents. Instead:
> - **Graph:** `FabricGQLBackend` sends GQL queries directly to the
>   Fabric Graph Model REST API (`/GraphModels/{id}/executeQuery`)
> - **Telemetry:** `FabricKQLBackend` sends KQL queries directly to
>   the Eventhouse via `KustoClient` (`azure-kusto-data` SDK)
>
> The agents (GraphExplorerAgent, TelemetryAgent) compose queries from
> their language prompts (`language_gql.md` for GQL, KQL instructions
> for telemetry) and submit them through the existing `/query/graph`
> and `/query/telemetry` endpoints — the backend dispatch layer routes
> to the appropriate Fabric backend based on the scenario's connector type.
>
> Data Agent prompts exist in `fabric_implementation_references/data/prompts/deprecated/`
> but are not used.

Reference implementation: `fabric_implementation_references/scripts/testing_scripts/test_kql_query.py`
(uses `KustoClient` + `DefaultAzureCredential`).
...
class FabricKQLBackend:
    """Telemetry backend for Fabric Eventhouse (KQL).

    Sends KQL queries directly to the Eventhouse via azure-kusto-data SDK.
    No Fabric Data Agent involved — direct KQL execution.
    """
...
### ~~F7: Fabric Data Agent discovery~~ — REMOVED

> **Not applicable.** The architecture does NOT use Fabric Data Agents.
> Agents send GQL/KQL queries directly to Fabric REST APIs. The
> `collect_fabric_agents.py` reference script and Data Agent prompts
> in `deprecated/` are historical artifacts from an earlier design.

### ~~F8: Data Agent UI in FabricSetupWizard~~ — REMOVED

> **Not applicable.** No Data Agent discovery or assignment needed.
> The Wizard's Step 2 shows Fabric resources (Lakehouse, Eventhouse,
> Ontology, Graph Model) — not Data Agents.
...
| File | Phase |
|------|-------|
| `data/scenarios/telco-noc-fabric/` (scenario.yaml + symlinked data dirs) | B |
...
| File | Phase | Change |
|------|-------|--------|
| `fabric_provision.py` (596 lines) | B | +~800 lines: data path resolver, CSV upload, ontology def, graph model discovery, conditional execution, idempotent retry |
