...
## Resource & Agent Flow Visualizer

> UI tab that visualises the multi-agent flow, data sources, and their connections.

### The Idea

A **"Resources"** tab (alongside "Investigate" and "Scenario Info") that
...
### Does the UI Actually Need to Change?
...
1. **We already have `react-force-graph-2d`** — the hardest part of any graph
   viz (layout, interaction, rendering) is solved.
...
3. **The resource graph is small** — a typical scenario has 5 agents + 4-5 data
   ...
   the topology graph (100+ nodes). Force layout will converge instantly.
...
4. **Existing component patterns are directly reusable** — `GraphCanvas.tsx`'s
   ...
   hover info — all adaptable with different node shapes/colors.

5. **No new dependencies needed** — everything is already in `package.json`.

#### The one caveat

**The backend data assembly.** The relationship between agents, tools,
and data sources is currently **implicit** — encoded in provisioning logic,
not exposed as queryable metadata. The frontend needs a single endpoint returning
the full resource graph (nodes + edges).

**Before genericization**: this endpoint would be hardcoded or assembled
from `agent_ids.json` + `scenario.yaml`. Doable but brittle.

**After genericization**: the scenario config YAML *already contains* all this
information. The endpoint becomes a trivial YAML-to-graph transformation.
**This is one of the payoffs of genericization.**

### Visual Language

| Node Type | Shape | Color | Example |
|-----------|-------|-------|---------|
| **Agent (orchestrator)** | Large circle, double border | Blue `#3b82f6` | Orchestrator |
| **Agent (sub-agent)** | Circle | Blue `#60a5fa` | GraphExplorerAgent |
| **Data Source** | Rounded rectangle | Green `#22c55e` | Cosmos Gremlin |
| **Tool** | Diamond | Orange `#f59e0b` | OpenAPI: query_graph |
| **Search Index** | Rounded rectangle | Purple `#a855f7` | runbooks-index |
...
Force-directed layout works well — orchestrator gravitates to center, sub-agents
form a ring, tools and data sources radiate outward. For a more structured view,
fix `node.fy` (vertical position) per node type using `d3.forceY()`:
...
|------|-----------|---------|
| `components/ResourceVisualizer.tsx` | ~250 | Main orchestrator component |
| `components/resource/ResourceCanvas.tsx` | ~200 | `ForceGraph2D` wrapper with resource-specific rendering |
| `components/resource/ResourceToolbar.tsx` | ~100 | Filter chips, search, layout toggle |
| `components/resource/ResourceTooltip.tsx` | ~60 | Hover tooltip for resource nodes |
| `components/resource/resourceConstants.ts` | ~30 | Node type colors, sizes, shapes |
| `hooks/useResourceGraph.ts` | ~50 | Fetch resource graph from API |
...
| File | Change |
|------|--------|
| `App.tsx` | Add `'resources'` to `AppTab`, add content branch |
| `TabBar.tsx` | Add third tab button |
| `types/index.ts` | Add `ResourceNode`, `ResourceEdge` types |

### Build Sequencing

**Option C was chosen** — build frontend with mock data now, connect to real
backend after genericization. Total throwaway code: ~20 lines of mock data
replaced by ~30 lines of fetch+transform.

### Visualizer Open Questions

1. **Runtime state overlay?** Light up agents during investigations as SSE
   events arrive. Cool but complex — probably V10.
2. **Inline prompt editing?** Could link from resource graph to SettingsModal.
   Nice-to-have, not essential.
3. **Topology graph vs resource graph**: Separate components sharing utilities
   is cleaner than one uber-component with mode switches.
...
| `src/components/resource/ResourceCanvas.tsx` | 220 | `ForceGraph2D` wrapper with shape-per-type rendering, layered y-force, directional arrows |
...
auto-stratify into layers via `d3.forceY` targets.
...
## Incremental Next Steps (Low-Risk, Certainly-Required)

> What can we do right now that is guaranteed to be needed, can be implemented
> and tested without much investment, and won't create throwaway work?

### Guiding Principles

Each incremental move must:
1. Be **certainly required** regardless of future design decisions
2. Can be **implemented and tested** in isolation (no half-built states)
3. Be **small enough** that it won't block anything if interrupted
4. Leave the app **fully working** after each step

### Candidate Steps

#### Step 1: Introduce `DocumentStore` Protocol (for NoSQL operations)

**What:** Define a `DocumentStore` Protocol (like the existing `GraphBackend`
Protocol) that abstracts Cosmos NoSQL CRUD+query operations. Implement a
`CosmosDocumentStore` adapter and optionally a `MockDocumentStore` for testing.

**Why certainly required:** Four routers (`router_telemetry`, `router_interactions`,
`router_scenarios`, `router_prompts`) all directly import `cosmos_helpers` and
call Cosmos SDK methods. Any new backend will need these same operations.

**Scope:**
- New `stores/__init__.py` — Protocol with `list()`, `get()`, `upsert()`,
  `delete()`, `query()` methods (~40 lines)
- New `stores/cosmos_nosql.py` — wraps existing `cosmos_helpers` (~80 lines)
- Optionally: `stores/mock_store.py` — in-memory dict (~60 lines)
- **No router changes yet** — just create the abstraction alongside existing code

**Risk:** Very low. Additive only — no existing code is modified.
**Effort:** ~180 lines new, 0 lines modified.

#### Step 2: Extract Cosmos Config into Adapter Config Module

**What:** Move all Cosmos-specific environment variables and credential setup
from `config.py` into a dedicated `adapters/cosmos_config.py`. Leave `config.py`
with only generic settings.

**Why certainly required:** `config.py` is 50% Cosmos connection strings that
are irrelevant for non-Cosmos backends. Clean separation is always needed.

**Scope:**
- New `adapters/cosmos_config.py` (~40 lines)
- Modify `config.py`, `cosmos_helpers.py`, `backends/cosmosdb.py` — update imports

**Risk:** Low. Straightforward import reshuffling.
**Effort:** ~40 lines new, ~50 lines modified across 4 files.

#### Step 3: Refactor `ScenarioContext` to Use Generic Field Names

**What:** Rename Cosmos-specific fields: `gremlin_database` → `graph_database`, etc.

**Why certainly required:** `ScenarioContext` leaks implementation details.
Any backend addition needs generic names.

**Risk:** Low but **not invisible** — field renames touch 4-5 files, ~15 occurrences.
Could cause `AttributeError` if any reference is missed.
**Effort:** ~25 lines modified across 5 files.

#### Step 4: Extract Blob + AI Search from `router_ingest.py`

**What:** Pull blob storage uploads and AI Search indexer creation into dedicated
`services/blob_uploader.py`. These are Azure service integrations that stay the
same regardless of graph backend.

**Risk:** Medium-low. SSE progress reporting is threaded through — callback
needs to be passed to the service.
**Effort:** ~60 lines new, ~80 lines modified.

#### Step 5: Add `ingest()` to `GraphBackend` Protocol

**What:** Extend `GraphBackend` Protocol with `ingest(vertices, edges)` so
graph data loading goes through the same abstraction as querying.

**Risk:** Medium. The current ingest code does a lot: SSE progress, batch
processing with retry, ARM resource creation. Interface needs careful design.
**Effort:** ~90 lines new, ~120 lines modified.

#### Step 6: Migrate One NoSQL Router to `DocumentStore`

**What:** Refactor `router_interactions.py` (146 lines, pure CRUD) to use
`DocumentStore` Protocol. Validates that the abstraction actually works.

**Risk:** Low. Simplest CRUD router, no business logic.
**Effort:** ~50 lines modified. Requires Step 1.

### Graph-Query-API Split Analysis

#### Already generic (no changes needed) — 896 lines (27%)

| File | Lines | Why |
|------|-------|-----|
| `backends/__init__.py` | 120 | `GraphBackend` Protocol + factory |
| `backends/mock.py` | 204 | Implements protocol with static data |
| `router_graph.py` | 73 | Uses `GraphBackend` exclusively |
| `router_topology.py` | 69 | Uses `GraphBackend` exclusively |
| `models.py` | 107 | Pure Pydantic, no Azure imports |
| `sse_helpers.py` | 86 | Pure asyncio/SSE |
| `main.py` | 237 | FastAPI app wiring — nearly generic |

#### Cosmos-specific (needs abstraction) — 2,330 lines (69%)
...
| `router_telemetry.py` | 144 | Cosmos SQL queries |
| `router_interactions.py` | 146 | Cosmos CRUD |
| `router_scenarios.py` | 220 | Cosmos CRUD + validation |
| `router_prompts.py` | 288 | Cosmos CRUD + versioning |
| `router_ingest.py` | 871 | Gremlin + NoSQL + Blob + Search (most tangled) |
| `search_indexer.py` | 226 | AI Search pipelines |

#### The honest take

No physical directory split is needed yet. What's needed is the **abstraction
boundary** — a `DocumentStore` Protocol between routers and Cosmos. Once that
exists, routers become generic-by-protocol (`store.query()` not
`container.query_items()`), and Cosmos lives behind the protocol.

The `GraphBackend` Protocol for graph operations is **already done and working**.
The missing piece is the equivalent pattern for NoSQL/document operations.
...
|-------|------|--------|-------------|
| **1** | `DocumentStore` Protocol + `CosmosDocumentStore` | Easy | None |
| **2** | Extract Cosmos config into adapter module | Easy | None |
| **3** | Rename `ScenarioContext` fields to generic names | Easy | None |
| **4** | Migrate `router_interactions` to `DocumentStore` | Easy | Step 1 |
| **5** | Extract blob/search into service modules | Medium | None |
| **6** | Migrate `router_scenarios` to `DocumentStore` | Easy | Step 1 |
| **7** | Migrate `router_telemetry` to `DocumentStore` | Medium | Step 1 |
| **8** | Add `ingest()` to `GraphBackend` Protocol | Medium-Hard | None |
| **9** | Migrate `router_prompts` to `DocumentStore` | Hard | Step 1 |
| **10** | Refactor `router_ingest` to use both protocols | Hard | Steps 1, 5, 8 |

Steps 1-3 can be done in parallel. Steps 4-7 in any order.
Steps 8-10 are the hard ones — wait until easier migrations validate the design.

### Safety Assessment

- **Step 1 (DocumentStore Protocol)**: Truly invisible. Purely additive — no
  existing code modified. Safe to ship.
- **Step 2 (Cosmos config extraction)**: Modifies imports in 3-4 files. Low risk
  but not zero — a missed import causes `ImportError` at startup.
- **Step 3 (ScenarioContext field renames)**: Low risk but carries real
  `AttributeError` potential if any reference is missed across 4-5 files.
- **Steps 4-10**: Each modifies existing router code. Requires testing the
  affected endpoints.

### What NOT to Do Yet

- **Don't restructure directories** (moving files into `core/`, `stores/`,
  `services/` sub-packages). Import path changes break everything downstream.
  Do the abstraction first, directory layout last.
- **Don't touch `router_ingest.py`** until easier routers prove the
  `DocumentStore` pattern works. It's the hardest file and should be last.
- **Don't build new backends** (Fabric, Neo4j) until the abstraction exists.
  Building them now means building to a moving target.