...
> **Audited:** 2026-02-20 — corrections applied (see [Audit Corrections](#audit-corrections))  
...
The "Data Ops" tab streams from two SSE endpoints whose backend log filters match **phantom logger names** (modules/prefixes that don't exist or are misspelled). On the API side, both prefixes (`app.routers.config` and `app.routers.fabric`) are phantom — config.py's logger is named `"app.config"`, not `"app.routers.config"`. On the graph-query-api side, `graph-query-api.cosmos` is the only valid logger, but it's already covered by the main `/query/logs` stream. The tab provides zero unique information.
...
| `graph-query-api/main.py` | Remove `_data_ops_broadcaster`, its handler, filter, and the `/query/logs/data-ops` endpoint (lines 153–165 + 178–185). **Do NOT remove lines 167–175** — those are the main `/query/logs` endpoint. |

**Detail on phantom logger names:**

| Endpoint | Filter Matches Logger | Logger Exists? |
|----------|----------------------|----------------|
| `/api/logs/data-ops` | `app.routers.config` | **NO** — config.py uses `getLogger("app.config")`, not `"app.routers.config"`. Filter never matches. |
...
| 2 | `api/app/sessions.py` L58 | Remove `_thread: Optional[threading.Thread] = field(default=None, repr=False)`. **Do NOT remove `import threading`** — `threading.Event` (L59, `_cancel_event`) and `threading.Lock` (L67, `_lock`) are actively used. | 1 | NONE |
| 3 | `api/app/main.py` L29 | Remove redundant `load_dotenv(os.path.join(..., "azure_config.env"))` call — already loaded by `paths.py` at import time (imported transitively via `app.routers.config` at L26). `load_dotenv` is idempotent, so this is a no-op. | 1 | NONE |
...
| 16 | `graph-query-api/adapters/fabric_config.py` L26, L41–45 | Remove 6 dead constants: `FABRIC_WORKSPACE_ID`, `FABRIC_WORKSPACE_NAME`, `FABRIC_LAKEHOUSE_NAME`, `FABRIC_EVENTHOUSE_NAME`, `FABRIC_ONTOLOGY_NAME`, `FABRIC_CAPACITY_ID`. Also remove `_require_env()` helper — all 4 callers are the dead constants above, zero remaining callers. **This also fixes a latent bug:** `_require_env()` for these vars runs at import time, crashing the app if they're unset — even though no code uses them. | ~25 | NONE (fixes bug) |
...
| `_data_ops_broadcaster` in `api/app/routers/logs.py` is fully useless | **CORRECT** | Both filter prefixes are phantom: `app.routers.fabric` (no such module) AND `app.routers.config` (config.py uses `getLogger("app.config")`, not `"app.routers.config"`). Zero loggers match the data-ops filter. The tab is completely non-functional. |
...
| `graph-query-api/main.py` data-ops at "lines 153–186" | **FIXED** | The original range included the main `/query/logs` endpoint (lines 167–175) that must be preserved. Corrected to **153–165 + 178–185** in Phase 2. |
| `app.routers.config` is a valid logger for data-ops | **WRONG** | The actual logger is `getLogger("app.config")`, not `"app.routers.config"`. The `startswith("app.routers.config")` filter in the data-ops endpoint matches **zero** loggers. Both API-side filter prefixes are phantom. |
| `import threading` can be removed from sessions.py | **WRONG** | `threading.Event` (`_cancel_event`, L59) and `threading.Lock` (`_lock`, L67) are actively used. Only the `_thread` field itself (L58) is dead. Fixed in Phase 3 Item 2. |
