...
> **Approach:** All enhancements are implemented in a **new scenario** called `telecom-playground` (copied from `telco-noc`). This leaves the baseline `telco-noc` untouched and deployable. Use `./deploy.sh --scenario telecom-playground` to deploy the enhanced version.

**Phase 1 (data-only — no code changes):** Scenarios 1 + 2 — new prompts and telemetry only, dramatically widen the demo repertoire.  
**Phase 2 (data + graph expansion — no code changes):** Scenario 4 (PhysicalConduit) — ~5 vertices, ~10 edges, unlocks the "hidden shared risk" narrative. All provisioning scripts are now data-driven from `graph_schema.yaml` and `scenario.yaml` — just add CSV files and update the YAML schema.  
**Phase 3 (medium expansion — no code changes):** Scenario 5 (predictive) — most impressive for exec audiences, requires sustained telemetry generation.
...
This section compares the **current** `telco-noc` ontology (baseline) against the **proposed enhanced** ontology for the new `telecom-playground` scenario after all three phases of enhancements (Section 10) are implemented.
...
| **Graph ingest pipeline** | No code changes — all provisioning scripts (`provision_lakehouse.py`, `provision_eventhouse.py`, `provision_ontology.py`) and the topology generator are data-driven from `graph_schema.yaml` and `scenario.yaml` | ✅ Yes |
...
| [scripts/fabric/provision_lakehouse.py](../scripts/fabric/provision_lakehouse.py#L42) | 42 | _(none)_ | Now data-driven from `graph_schema.yaml` — no `telco-noc` references |
| [scripts/fabric/provision_eventhouse.py](../scripts/fabric/provision_eventhouse.py#L38) | 38 | _(none)_ | Now data-driven from `scenario.yaml` — no `telco-noc` references |
...
> **Prerequisites:** The `telco-noc` scenario exists and is the current baseline. This plan creates a **new** scenario called `telecom-playground` by copying `telco-noc` and making all modifications in the copy. The original `telco-noc` is left untouched.

### Phase 1: Data & Schema Expansion

Phase 1 starts by copying the existing `telco-noc` scenario to create the new `telecom-playground` scenario directory. All subsequent steps create new CSV files, update the graph schema, update the agent prompt, and add new example questions — all within `data/scenarios/telecom-playground/`.

---

#### Step 0 — Create the `telecom-playground` scenario (copy from `telco-noc`)

**Action:** Copy the entire `telco-noc` scenario directory to create the new scenario:

```bash
cp -r data/scenarios/telco-noc data/scenarios/telecom-playground
```

Then update the internal scenario identity in `data/scenarios/telecom-playground/scenario.yaml`:

| Field | Old Value | New Value |
|---|---|---|
| `name` | `telco-noc` | `telecom-playground` |
| `display_name` | `"Australian Telco NOC — Fibre Cut Incident"` | `"Telecom Playground — Extended Telco Demo"` |
| `graph.resource_name` | `telco-noc-topology` | `telecom-playground-topology` |
| `container_prefix` | `telco-noc` | `telecom-playground` |
| `search_indexes.runbooks.index_name` | `telco-noc-runbooks-index` | `telecom-playground-runbooks-index` |
| `search_indexes.tickets.index_name` | `telco-noc-tickets-index` | `telecom-playground-tickets-index` |

**Verify:**
```bash
ls data/scenarios/telecom-playground/scenario.yaml   # exists
grep '^name:' data/scenarios/telecom-playground/scenario.yaml   # telecom-playground
grep 'index_name:' data/scenarios/telecom-playground/scenario.yaml   # telecom-playground-*-index
```

> **Note:** Because the provisioning scripts and `deploy.sh` are fully data-driven from `scenario.yaml` and `graph_schema.yaml`, using `--scenario telecom-playground` at deploy time automatically creates separate Fabric resources (Lakehouse tables, Eventhouse, ontology) and AI Search indexes with the `telecom-playground` prefix. The `telco-noc` deployment is completely independent.

---

#### Step 1.1 — Add `FirmwareVersion` column to `DimCoreRouter.csv`

**File:** `data/scenarios/telecom-playground/data/entities/DimCoreRouter.csv`
...
**Verify:** `head -1 data/scenarios/telecom-playground/data/entities/DimCoreRouter.csv` shows `RouterId,City,Region,Vendor,Model,FirmwareVersion`.

---

#### Step 1.2 — Create `DimPhysicalConduit.csv`

**File:** `data/scenarios/telecom-playground/data/entities/DimPhysicalConduit.csv` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/entities/FactConduitMapping.csv` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/entities/DimAmplifierSite.csv` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/entities/FactAmplifierMapping.csv` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/entities/DimAdvisory.csv` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/entities/FactAdvisoryMapping.csv` (NEW)
...
**File:** `data/scenarios/telecom-playground/graph_schema.yaml`
...
**File:** `data/scenarios/telecom-playground/scenario.yaml`
...
**Verify:** `grep -c 'color:' data/scenarios/telecom-playground/scenario.yaml` returns 11 (8 original + 3 new).
...
**Files:**
- `data/scenarios/telecom-playground/scenario.yaml` (authoritative — loaded at runtime)
...
**Verify:** `grep -c '^\s*-' data/scenarios/telecom-playground/scenario.yaml` in the `example_questions:` block shows **9** entries (6 original + 3 new). `exampleQuestions` in `config.ts` has **9** entries.
...
|---|---|---|---|
| 1 | CSV file count | `ls data/scenarios/telecom-playground/data/entities/*.csv \| wc -l` | **16** (was 10) |
| 2 | DimCoreRouter has FirmwareVersion | `head -1 data/scenarios/telecom-playground/data/entities/DimCoreRouter.csv` | Header includes `FirmwareVersion` |
| 3 | FIBRE-01 and FIBRE-02 share conduit | `grep CONDUIT-SYD-MEL-INLAND data/scenarios/telecom-playground/data/entities/FactConduitMapping.csv \| wc -l` | **2** (both fibres) |
| 4 | Advisory affects correct routers | `grep CORE-SYD-01 data/scenarios/telecom-playground/data/entities/FactAdvisoryMapping.csv \| wc -l` | **2** (two advisories affect SYD-01) |
| 5 | graph_schema.yaml vertex count | `awk '/^vertices:/,/^edges:/{if (/^  - label:/) c++} END{print c}' data/scenarios/telecom-playground/graph_schema.yaml` | **11** |
| 6 | core_schema.md has new types | `grep -c '### PhysicalConduit\|### AmplifierSite\|### Advisory' data/scenarios/telecom-playground/data/prompts/graph_explorer/core_schema.md` | **3** |
| 7 | core_schema.md has new rels | `grep -c 'routed_through\|amplifies\|affects_version' data/scenarios/telecom-playground/data/prompts/graph_explorer/core_schema.md` | **3+** |
| 8 | scenario.yaml has 11 node styles | `grep -c 'color:' data/scenarios/telecom-playground/scenario.yaml` | **11** |
| 9 | config.ts nodeColors count | `grep -A20 'nodeColors:' frontend/src/config.ts \| grep -c '"#'` | **11** |
| 10 | exampleQuestions count | `grep -c '^\s*-' data/scenarios/telecom-playground/scenario.yaml` under `example_questions:` | **9** |

---

### Phase 2: Knowledge Documents, Prompts & Deploy

Phase 2 covers the changes needed to make the new data work in the deployed system:

1. **Knowledge documents** for AI Search (new runbooks auto-discovered by the indexer)
2. **Demo scenario prompts** for new investigative flows
3. **Frontend rebuild** (if Step 1.11 was done)
4. **Deployment** with correct pipeline sequence

> **Important:** All provisioning scripts (`provision_lakehouse.py`, `provision_eventhouse.py`, `provision_ontology.py`) and the topology generator (`generate_topology_json.py`) are now **fully data-driven** from `graph_schema.yaml` and `scenario.yaml`. They require **zero code changes** when adding new entity types, relationships, or telemetry tables. The only Phase 1 data changes that matter to the pipeline are the YAML schema edits and CSV files.

---

#### Step 2.1 — Create advisory knowledge documents for AI Search

**Directory:** `data/scenarios/telecom-playground/data/knowledge/runbooks/`

**Action:** Create these new runbook/advisory documents:

**File:** `data/scenarios/telecom-playground/data/knowledge/runbooks/conduit_shared_risk_assessment.md` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/knowledge/runbooks/amplifier_maintenance.md` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/knowledge/runbooks/firmware_upgrade_procedure.md` (NEW)
...
**Verify:** `ls data/scenarios/telecom-playground/data/knowledge/runbooks/*.md | wc -l` returns **8** (was 5, +3 new).

---

#### Step 2.2 — Create demo scenario prompt files

**Directory:** `data/scenarios/telecom-playground/data/prompts/demo_scenarios/` (subdirectory to distinguish from agent system prompts)
...
**File:** `data/scenarios/telecom-playground/data/prompts/demo_scenarios/planned_maintenance.md` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/prompts/demo_scenarios/conduit_correlation.md` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/prompts/demo_scenarios/predictive_degradation.md` (NEW)
...
**File:** `data/scenarios/telecom-playground/data/prompts/demo_scenarios/firmware_advisory_correlation.md` (NEW)
...
**Verify:** `ls data/scenarios/telecom-playground/data/prompts/demo_scenarios/*.md | wc -l` returns **4**.
...
```bash
./deploy.sh --scenario telecom-playground --provision-all --yes
```
...
> **Note:** `--scenario telecom-playground` tells `deploy.sh` to use the new scenario directory. `--provision-all` is required to enable Steps 5–7 (Fabric, data, agent provisioning). `--yes` skips confirmation prompts.
...
uv run python scripts/generate_topology_json.py --scenario telecom-playground
...
# 4. Upload CSVs + rebuild Lakehouse tables (auto-discovers tables from graph_schema.yaml)
uv run python scripts/fabric/provision_lakehouse.py

# 5. Re-create ontology (auto-discovers entity/relationship types from graph_schema.yaml)
...
| # | Check | How | Expected |
|---|---|---|---|
| 1 | Runbook count | `ls data/scenarios/telecom-playground/data/knowledge/runbooks/*.md \| wc -l` | **8** |
| 2 | Demo prompt count | `ls data/scenarios/telecom-playground/data/prompts/demo_scenarios/*.md \| wc -l` | **4** |
...
|---|---|---|---|
| **CREATED** | `data/scenarios/telecom-playground/` (copied from `telco-noc`) | 1 | 0 |
| **MODIFIED** | `data/scenarios/telecom-playground/scenario.yaml` (rename identifiers) | 1 | 0 |
| **MODIFIED** | `data/scenarios/telecom-playground/data/entities/DimCoreRouter.csv` | 1 | 1.1 |
| **CREATED** | `data/scenarios/telecom-playground/data/entities/DimPhysicalConduit.csv` | 1 | 1.2 |
| **CREATED** | `data/scenarios/telecom-playground/data/entities/FactConduitMapping.csv` | 1 | 1.3 |
| **CREATED** | `data/scenarios/telecom-playground/data/entities/DimAmplifierSite.csv` | 1 | 1.4 |
| **CREATED** | `data/scenarios/telecom-playground/data/entities/FactAmplifierMapping.csv` | 1 | 1.5 |
| **CREATED** | `data/scenarios/telecom-playground/data/entities/DimAdvisory.csv` | 1 | 1.6 |
| **CREATED** | `data/scenarios/telecom-playground/data/entities/FactAdvisoryMapping.csv` | 1 | 1.7 |
| **MODIFIED** | `data/scenarios/telecom-playground/graph_schema.yaml` | 1 | 1.8 |
| **MODIFIED** | `data/scenarios/telecom-playground/data/prompts/graph_explorer/core_schema.md` | 1 | 1.9 |
| **MODIFIED** | `data/scenarios/telecom-playground/scenario.yaml` | 1 | 1.10, 1.12 |
| **MODIFIED** | `frontend/src/config.ts` (optional) | 1 | 1.11, 1.12 |
| **CREATED** | `data/scenarios/telecom-playground/data/knowledge/runbooks/conduit_shared_risk_assessment.md` | 2 | 2.1 |
| **CREATED** | `data/scenarios/telecom-playground/data/knowledge/runbooks/amplifier_maintenance.md` | 2 | 2.1 |
| **CREATED** | `data/scenarios/telecom-playground/data/knowledge/runbooks/firmware_upgrade_procedure.md` | 2 | 2.1 |
| **CREATED** | `data/scenarios/telecom-playground/data/prompts/demo_scenarios/planned_maintenance.md` | 2 | 2.2 |
| **CREATED** | `data/scenarios/telecom-playground/data/prompts/demo_scenarios/conduit_correlation.md` | 2 | 2.2 |
| **CREATED** | `data/scenarios/telecom-playground/data/prompts/demo_scenarios/predictive_degradation.md` | 2 | 2.2 |
| **CREATED** | `data/scenarios/telecom-playground/data/prompts/demo_scenarios/firmware_advisory_correlation.md` | 2 | 2.2 |
| **Total** | **5 modified + 15 created = 20 files** (all within `telecom-playground/`, `telco-noc/` untouched) | |
...
|---|---|---|---|---|
| 1 | ~~**CRITICAL**~~ | ~~`provision_lakehouse.py` missing 6 new table entries~~ | **Resolved by decoupling** — script now auto-discovers tables from `graph_schema.yaml` | — |
| 2 | ~~**CRITICAL**~~ | ~~`provision_ontology.py` missing ~150 lines (IDs, entity types, bindings, contextualizations)~~ | **Resolved by decoupling** — script now auto-discovers entity/relationship types from `graph_schema.yaml` | — |
| 3 | **HIGH** | ADV-CISCO-2025-002 `AffectedVersions` missing `IOS-XR-7.9.2` | Fixed in Step 1.6 CSV content | 1.6 |
| 4 | **HIGH** | Deploy step missing `provision_search_index.py` re-run | Step 2.4 Deploy now shows full pipeline (Option A: `./deploy.sh`, Option B: manual sequence including indexer) | 2.4 |
| 5 | **HIGH** | `generate_topology_json.py` not re-run after schema changes | Noted that `deploy.sh` Step 2b auto-runs it; also included in Option B manual sequence | 2.4 |
| 6 | **HIGH** | New prompt files have no consumer / integration point | Step 2.2 clarifies they are "demo starters" for copy-paste, placed in `demo_scenarios/` subdirectory | 2.2 |
...
| 11 | **LOW** | Demo prompt files mixed with agent system prompts | Step 2.2 now uses `prompts/demo_scenarios/` subdirectory | 2.2 |

> **All 11 issues resolved.** The plan is now implementation-ready. Step 0 copies `telco-noc` to create `telecom-playground`. Phase 1 (Steps 1.1–1.13) covers data, schema, prompt, and config changes within the new scenario. Phase 2 (Steps 2.1–2.5) covers knowledge documents, demo prompts, frontend rebuild, and deployment with `--scenario telecom-playground`. Provisioning scripts are fully data-driven — no code changes needed. The original `telco-noc` scenario remains untouched.