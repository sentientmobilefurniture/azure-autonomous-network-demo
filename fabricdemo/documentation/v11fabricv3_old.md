...
> **Created:** 2026-02-16
> **Status:** 🔶 In progress (v11fabricprepa.md + v11fabricprepb.md + v11d.md implemented)
...
> v11d.md has been completed (AgentBar, resizable panels, gear removal, SettingsModal deletion)
> **Scope:** Fix the broken Fabric UX end-to-end. Backend bug fixes, provision
> pipeline completion, UI restructure (~~kill SettingsModal~~ ✅ v11d, add ConnectionsDrawer),
...
- ~~`SettingsModal.tsx` — Fabric tab (conditional on active scenario) ✅~~ _(deleted by v11d; replaced by `FabricSetupModal.tsx` wrapping `FabricSetupTab.tsx`)_
...
| #15: Generic `_find_or_create()` | `_find_or_create()` in `fabric_provision.py` (596 lines after v11fabricprepb lock addition), 4 functions delegate to it | Phase B data-upload steps slot in cleanly |
...
| #32: SettingsModal split | ~~`settings/` dir with 4 tab components; SettingsModal is 201 lines~~ **v11d update:** SettingsModal.tsx deleted, 3 tab files deleted. Only `settings/FabricSetupTab.tsx` remains, wrapped by new `FabricSetupModal.tsx`. | ScenarioManagerModal composes from FabricSetupTab + other pieces |
...
### What v11d (Agent Bar & Resizable Panels) changed

All 8 v11d phases are complete. Key changes affecting this plan:

| v11d item | Current state | Impact on this plan |
|---|---|---|
| Gear button removed from Header | Header.tsx is now 42 lines: title + ScenarioChip + conditional ⬡ Fabric button + AgentBar + ProvisioningBanner | ✅ P6 ("⚙ gear is a junk drawer") partially resolved — gear gone. ConnectionsDrawer still needed for comprehensive settings entry point. |
| HealthDot moved to AgentBar | `HealthDot.tsx` still exists, imported by `AgentBar.tsx` (not Header) | ✅ P5 partially resolved — ambient API health visible in AgentBar. Service-level health (ConnectionsDrawer) still needed. |
| AgentBar + AgentCard created | `AgentBar.tsx`, `AgentCard.tsx` — shows provisioned agents with hover tooltips | ConnectionsDrawer/ServiceHealthSummary must coexist with AgentBar in visual stack: Header → AgentBar → ProvisioningBanner → content |
| SettingsModal.tsx deleted | Only backup copy exists. 3 of 4 `settings/` tab files also deleted. | ⚠️ Phase E task E3 is **mostly done** — only `settings/FabricSetupTab.tsx` remains (used by `FabricSetupModal.tsx`). Stale comment references in 3 files. |
| FabricSetupModal.tsx created | Wraps `FabricSetupTab.tsx` in `ModalShell`. Opened by Header’s conditional "⬡ Fabric" button (visible only when `isFabricScenario`). | ⚠️ P2 persists in modified form — Fabric settings still require an active Fabric scenario. Will be subsumed by ConnectionsDrawer (Phase C). |
| Resizable panels | `react-resizable-panels` wired into `App.tsx` for all content boundaries | No direct impact on Fabric plan. ConnectionsDrawer is a slide-over, not a panel. |
| ScenarioChip backend badge | `backendBadge()` shows Fabric (cyan), Mock (yellow), Cosmos (emerald) | ✅ C7 partial — badge done. Remaining: skeleton state only. |
...
### P2: Fabric tab is invisible until you already have a Fabric scenario _(partially changed by v11d)_

~~Old mechanism: gear icon → SettingsModal → Fabric tab (hidden unless Fabric scenario active).~~

**Current state (post-v11d):** SettingsModal.tsx deleted. `FabricSetupModal.tsx` wraps
`FabricSetupTab.tsx` and is opened by Header’s conditional "⬡ Fabric" button:
```typescript
isFabricScenario = activeScenarioRecord?.graph_connector === 'fabric-gql'
```
The chicken-and-egg problem persists: the Fabric button only appears for Fabric scenarios,
but you need Fabric resources to create one. **ConnectionsDrawer (Phase C) resolves this**
by providing Fabric workspace setup independent of the active scenario.
...
### P5: No visibility into connected services _(partially improved by v11d)_

~~The only health indicator was a small green dot labeled "API" checking `/health`.~~

**Current state (post-v11d):** HealthDot moved from Header to AgentBar. AgentBar shows
each provisioned agent as a card with health dot, name, and hover tooltip. API health
is still visible via HealthDot in AgentBar. However, there’s still no way to see
CosmosDB, Blob, AI Search, Foundry, or Fabric status. **ConnectionsDrawer (Phase C)
resolves this** with grouped service health.

### P6: ~~The ⚙ gear is a junk drawer~~ ✅ Resolved (v11d) — but replacement incomplete

~~SettingsModal was split into `settings/` tab components (201 lines shell + 4 tab files)
but still behind a gear icon with no discoverability.~~

**Current state (post-v11d):** Gear button removed. SettingsModal.tsx deleted. 3 of 4
`settings/` tab files deleted. Only `FabricSetupTab.tsx` remains, accessed via
`FabricSetupModal.tsx` (conditional on Fabric scenario). **The replacement UX
(ConnectionsDrawer + ScenarioManagerModal) is not yet built.** This means:
- Fabric settings: accessible only for Fabric scenarios via "⬡ Fabric" button
- Scenario settings / data source bindings: **no current entry point** (was in SettingsModal)
- Upload settings: moved to AddScenarioModal’s `useScenarioUpload`

**ScenarioManagerModal (Phase D) + ConnectionsDrawer (Phase C) will fully replace
the old SettingsModal functionality.**
...
`api/app/routers/fabric_provision.py` (596 lines) creates empty resource
...
```
BEFORE (pre-v11d)                     CURRENT (post-v11d)
──────                                ──────
Header:                               Header (42 lines):
  ◆ Title  [ScenarioChip ▾]  •API ⚙    ◆ Title  [ScenarioChip ▾]  [⬡ Fabric]
                                      AgentBar: [⬡ Orchestrator] [GraphExplorer] [Telemetry] •API
                                      ProvisioningBanner (conditional)

⚙ → SettingsModal (DELETED by v11d)   [⬡ Fabric] → FabricSetupModal (Fabric scenarios only)
  ├─ ScenarioSettingsTab (DELETED)     ScenarioChip dropdown:
  ├─ DataSourceSettingsTab (DELETED)     ├─ Scenario quick-select (with backend badges ✅)
  ├─ UploadSettingsTab (DELETED)         ├─ ✦ Custom mode
  └─ FabricSetupTab (still exists)       ├─ + New Scenario
                                         ├─ ⊞ Manage scenarios… ✅ (handler not wired)
                                         └─ (ScenarioManagerModal will be wired in Phase D)

                                       PLANNED (this plan)
                                       ──────
                                       Header:
                                         ◆ Title  [ScenarioChip ▾]  •5/5  🔌
                                       (⬡ Fabric button absorbed into ConnectionsDrawer)

Loading overlay (blocks UI 5s)         🔌 → ConnectionsDrawer (slide-over)
  "Validating scenario..."               ├─ Service health grid (grouped by role)
                                         ├─ Expandable rows with details
                                         └─ Fabric section (3-state, 5 resource types, provision)

                                       Non-blocking startup:
                                         Init from localStorage → background validate
```

Available building blocks from completed refactor + v11d:
...
- `FabricSetupTab.tsx` — only remaining settings/ tab, used by FabricSetupModal
- `AgentBar.tsx` + `AgentCard.tsx` — agent visibility strip (v11d)
- `FabricSetupModal.tsx` — standalone Fabric settings modal (v11d)

### Change 1: ~~Kill the ⚙ gear~~ ✅ Done (v11d) — extend the ScenarioChip dropdown

⚙ gear already removed by v11d. SettingsModal already deleted. "⊞ Manage scenarios…"
already added to ScenarioChip dropdown (v11fabricprepb). Backend badges already on
chip (v11d). **Remaining: wire ⊞ handler to ScenarioManagerModal (Phase D), skeleton
state while loading.**
...
**Current state (post-v11d):** HealthDot is in AgentBar (not Header). AgentBar shows
agent cards + API health dot. The change now is:
- Add `<ServiceHealthSummary />` to Header: shows "5/5 Services" with aggregate color
- Polls `GET /api/services/health` every 30s
- Clickable → opens ConnectionsDrawer
- Add 🔌 connections button → also opens ConnectionsDrawer
- Header's conditional "⬡ Fabric" button is absorbed into ConnectionsDrawer's Fabric section
  (remove from Header, Fabric settings accessible via ConnectionsDrawer regardless of scenario)
...
### Change 4: ScenarioManagerModal (replaces deleted SettingsModal)
...
`FabricSetupTab.tsx`. The shell is 201 lines.~~

**Current state (post-v11d):** SettingsModal.tsx and 3 tab files
(`ScenarioSettingsTab`, `DataSourceSettingsTab`, `UploadSettingsTab`) are **deleted**.
Only `settings/FabricSetupTab.tsx` remains (used by `FabricSetupModal.tsx`).

ScenarioManagerModal must be built from scratch using `<ModalShell>` + `<BindingCard>`.
It does NOT compose from the old tab pieces (they're gone). Simplified to 2 sections:
`Scenarios | Upload`. Fabric settings moved to ConnectionsDrawer (not in this modal).
...
|-----------|-----------|-------------|
| `ConnectionsDrawer.tsx` | ~320 lines | Slide-over with grouped services + Fabric section (workspace setup UI + Data Agent section). Absorbs `FabricSetupModal.tsx` functionality. Uses `<ModalShell>`, `<ProgressBar>`. |
| `ServiceHealthSummary.tsx` | ~60 lines | "N/M Services" ambient indicator in header |
| `ScenarioManagerModal.tsx` | ~400 lines | Built from scratch with `<ModalShell>` + `<BindingCard>` (old tab components are deleted) |

### Modified components

| Component | Changes |
|-----------|---------|
| `Header.tsx` | ~~Remove ⚙ gear~~ ✅ Done (v11d). Add ServiceHealthSummary + 🔌 button. Remove conditional "⬡ Fabric" button (absorbed into ConnectionsDrawer). Header is now 42 lines with AgentBar. |
| `ScenarioChip.tsx` | ~~Add "⊞ Manage" action~~ ✅ Done (v11fabricprepb), ~~backend badge~~ ✅ Done (v11d). Remaining: skeleton state + wire ⊞ handler to ScenarioManagerModal. Uses `useClickOutside`. |
...
|-----------|-------|--------|
| ~~`SettingsModal.tsx` (201-line shell)~~ | ~~E~~ | ✅ **Already deleted by v11d** |
| ~~`settings/ScenarioSettingsTab.tsx`~~ | ~~E~~ | ✅ **Already deleted by v11d** |
| ~~`settings/DataSourceSettingsTab.tsx`~~ | ~~E~~ | ✅ **Already deleted by v11d** |
| ~~`settings/UploadSettingsTab.tsx`~~ | ~~E~~ | ✅ **Already deleted by v11d** |
| `settings/FabricSetupTab.tsx` | E | Still exists (used by `FabricSetupModal.tsx`). Delete when ConnectionsDrawer subsumes Fabric settings. |
| `FabricSetupModal.tsx` | E | Created by v11d. Delete when ConnectionsDrawer replaces it. |
| Stale "SettingsModal" comment refs | E | 3 files: `ModalShell.tsx` L18, `sseStream.ts` L9, `triggerProvisioning.ts` L6 |
...
| C6: Update Header (~~remove gear~~ ✅ v11d, add health + connections, remove ⬡ Fabric button) | `Header.tsx` | Low (1hr) — Header now 42 lines, gear already gone, AgentBar in place. Remove conditional "⬡ Fabric" button (absorbed into ConnectionsDrawer). |
...
| Task | Files | Effort |
|------|-------|--------|
| D1: ScenarioManagerModal (build from scratch) | New `ScenarioManagerModal.tsx` | Medium (3hr) — uses `<ModalShell>`, `<BindingCard>` (old tab components are deleted, build fresh) |
...
| E3: ~~Delete SettingsModal + 3 tab files~~ ✅ Done (v11d). Remaining: delete `FabricSetupTab.tsx` + `FabricSetupModal.tsx` (after ConnectionsDrawer absorbs Fabric settings) + cleanup 3 stale comment references | `settings/FabricSetupTab.tsx`, `FabricSetupModal.tsx`, comments in `ModalShell.tsx`/`sseStream.ts`/`triggerProvisioning.ts` | Low (30min) |
...
> **Codebase state after refactor + v11d:** SettingsModal.tsx is **deleted** (was 201 lines).
> 3 of 4 `settings/` tab files deleted; only `FabricSetupTab.tsx` remains (used by
> `FabricSetupModal.tsx`). AddScenarioModal is 514 lines with `useScenarioUpload.ts`
> extracted. `router_ingest.py` is a 2-line re-export; actual code in `ingest/`
> package (7 modules). `fabric_provision.py` is 596 lines with generic
> `_find_or_create()` and `sse_provision_stream()`. Shared components exist:
> `ModalShell`, `ProgressBar`, `BindingCard`, `useClickOutside`,
> `triggerProvisioning`, `useScenarioUpload`. v11d additions: `AgentBar.tsx`,
> `AgentCard.tsx`, `FabricSetupModal.tsx`. ScenarioContext has
...
| File | Phase | Size est. |
|------|-------|-----------|
| `frontend/src/components/ConnectionsDrawer.tsx` | C+F | ~320 lines (workspace setup UI + Data Agent section; absorbs FabricSetupModal functionality) |
| `frontend/src/components/ServiceHealthSummary.tsx` | C | ~60 lines |
| `frontend/src/components/ScenarioManagerModal.tsx` | D | ~400 lines (built from scratch, not from old tab pieces) |
...
| File | Phase | Change |
|------|-------|--------|
| `api/app/routers/fabric_provision.py` (596 lines) | B | +~800 lines (data upload, ontology def, graph model discovery, conditional execution). Uses existing `_find_or_create()` and `sse_provision_stream()`. Concurrency lock ✅ Done (v11fabricprepb). |
...
| `Header.tsx` | C | ~~Remove gear~~ ✅ Done (v11d). Add ServiceHealthSummary + connections button. Remove "⬡ Fabric" button (absorbed into ConnectionsDrawer). Header now 42 lines with AgentBar. |
| `ScenarioChip.tsx` | C | ~~Add manage action~~ ✅ Done (v11fabricprepb), ~~badge~~ ✅ Done (v11d). Remaining: skeleton state + wire ⊞ handler. |
...
| ~~`settings/DataSourceSettingsTab.tsx`~~ | ~~E~~ | ✅ **Already deleted by v11d** |
| ~~`settings/UploadSettingsTab.tsx`~~ | ~~E~~ | ✅ **Already deleted by v11d** |
| `settings/FabricSetupTab.tsx` | E | Still exists. Delete after ConnectionsDrawer absorbs Fabric settings. |
| `FabricSetupModal.tsx` | E | Created by v11d. Delete after ConnectionsDrawer replaces it. |
| `HealthDot.tsx` | E | Still exists (used by `AgentBar.tsx`). Evaluate: keep for AgentBar or inline. |
| Stale comment refs | E | `ModalShell.tsx` L18, `sseStream.ts` L9, `triggerProvisioning.ts` L6 |
