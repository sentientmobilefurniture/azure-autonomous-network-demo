...
Oh. Also, the form for adding the new scenario should continue running even when you close it. I'm thinking there should be a scenario status tab where you can see the current status of scenarios being uploaded. Like you see the scenario name, and then below it in a tree structure type deal, you see the individual components and their individual status, and the current overall status of the uploaded scenario is up above. And this status could be saved in a cosmosDB? is that appropriate?
