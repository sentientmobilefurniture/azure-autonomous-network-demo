...
export function StepVisualizationModal({
  ...
  const hasStructuredViz = vizData && (vizData.type === 'graph' || vizData.type === 'table');
  const [activeTab, setActiveTab] = useState<ModalTab>('visualization');

  // Reset tab when modal opens
  useEffect(() => {
    if (isOpen) setActiveTab('visualization');
  }, [isOpen]);
  ...
  const queryText =
    vizData?.type === 'graph' ? vizData.data.query
    : vizData?.type === 'table' ? vizData.data.query
    : null;
  ...
  const title =
    vizData?.type === 'graph' ? 'Graph Query Results'
    : vizData?.type === 'table' ? 'Telemetry Data'
    : 'Agent Results';
              ...
              {hasStructuredViz && step.response && (
                <div className="flex gap-1 px-4 pt-3 pb-0 shrink-0">
                  <div className="flex gap-1 p-0.5 bg-neutral-bg3 rounded-lg w-fit">
                    ...
                    >
                      {vizData?.type === 'graph' ? 'Graph & Data' : 'Data Table'}
                    </button>
                    <button
              ...
              <div className="flex-1 overflow-auto min-h-[300px]">
                ...
                {!loading && !error && vizData && (
                  <>
                    ...
                    {activeTab === 'visualization' && (
                      <>
                        {vizData.type === 'graph' && <GraphResultView data={vizData} />}
                        {vizData.type === 'table' && <TableResultView data={vizData} />}
                        {vizData.type === 'documents' && <DocumentResultView data={vizData} />}
                      </>
                    )}
