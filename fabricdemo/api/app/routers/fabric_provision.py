...
def _resolve_asset_name(override: str | None, scenario_name: str, asset_type: str) -> str:
    ...
    Note: Fabric Lakehouse/Eventhouse/Ontology names do NOT allow hyphens.
    Hyphens are replaced with underscores in the generated name.
    """
    if override:
        return override
    if scenario_name:
        safe_name = scenario_name.replace("-", "_")
        return f"{safe_name}_{asset_type}"
    env_map = {
...
async def _find_or_create_ontology(
    client: AsyncFabricClient,
    workspace_id: str,
    ontology_name: str,
    parts: list[dict] | None = None,
) -> tuple[str, bool]:
    """Find or create an ontology. Returns (ontology_id, was_created).

    When creating, the definition parts are included inline in the creation
    request (matching Fabric's expected pattern). This ensures the ontology
    is fully initialized with entity types, relationships, and data bindings
    from the start — without a separate updateDefinition call.

    Args:
        parts: Full definition parts array. Required for creation;
               if None and ontology doesn't exist, creates without definition.

    Returns:
        Tuple of (ontology_id, was_created). Caller can skip updateDefinition
        if was_created is True (definition already applied).
    """
    # --- Find existing ---
    found_id: str | None = None
    try:
        data = await client.get(f"/workspaces/{workspace_id}/ontologies")
        for item in data.get("value", []):
        ...
        try:
            ...
            for item in data.get("value", []):
                if item.get("type") == "Ontology" and item.get("displayName") == ontology_name:
                    found_id = item["id"]
                    break
        except Exception:
            pass
    ...
    if found_id:
        logger.info("Found existing ontology: %s (%s)", ontology_name, found_id)
        return found_id, False

    # --- Create with definition inline ---
    create_body: dict = {
        "displayName": ontology_name,
        "description": "Network topology ontology for Autonomous Network Demo",
    }
    if parts:
        create_body["definition"] = {"parts": parts}

    status, headers, resp_body = await client.post(
        f"/workspaces/{workspace_id}/ontologies", create_body,
    )
    if status == 201:
        new_id = resp_body.get("id", "")
        logger.info("Created ontology with definition: %s (%s)", ontology_name, new_id)
        return new_id, True
    elif status == 202:
        result = await client.wait_for_lro(headers, "create ontology", timeout=600)
        new_id = (result or {}).get("id", "")
        logger.info("Created ontology (LRO): %s (%s)", ontology_name, new_id)
        return new_id, True
    else:
        raise HTTPException(
            status_code=status,
            detail=f"Failed to create ontology: {resp_body}",
        )
        ...
        async with _fabric_provision_lock:
            ...
            try:
                ...
                if graph_connector == "fabric-gql":
                    ...
                    ontology_id, was_created = await _find_or_create_ontology(
                        client, workspace_id, ontology_name, parts=parts,
                    )
                    completed_steps.append("ontology_create")

                    if not was_created:
                        # Existing ontology — update its definition
                        yield _sse_event("progress", {
                            "step": "ontology_def", "detail": "Updating existing ontology definition…", "pct": 75,
                        })
...
async def provision_ontology(req: OntologyRequest):
    ...
    async def _work(client):
        ...
        ontology_id, _created = await _find_or_create_ontology(
    ...
    async def _work(client: AsyncFabricClient):
        async with _fabric_provision_lock:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Step 5: Build ontology definition + create/update in one shot
                yield _sse_event("progress", {
                    "step": "ontology", "detail": "Building ontology definition…", "pct": 55,
                })
                parts = _build_ontology_definition(workspace_id, lakehouse_id, ontology_name)
                ontology_id, was_created = await _find_or_create_ontology(
                    client, workspace_id, ontology_name, parts=parts,
                ...
                yield _sse_event("progress", {
                    "step": "ontology",
                    "detail": f"Ontology {'created' if was_created else 'found'}: {ontology_id}",
                    "pct": 60,
                })

                if not was_created:
                    # Existing ontology — apply updated definition
                    yield _sse_event("progress", {
                        "step": "ontology_def", "detail": "Updating existing ontology definition…", "pct": 65,
                    })
                    await _apply_ontology_definition(client, workspace_id, ontology_id, parts)
                yield _sse_event("progress", {
                    "step": "ontology_def",
