...
def _load_current_config() -> dict:
    ...
    _current_config = {
        "graph": os.getenv("DEFAULT_SCENARIO", "telco-noc"),
...
class ConfigApplyRequest(BaseModel):
    ...
    prompt_scenario: str | None = None     # scenario name to load prompts from store
        ...
        async def run_provisioning():
            async with _provisioning_lock:
                try:
                    ...
                    # Resolve prompts — fetch from store if available
                    ...
                    if not prompts:
                        ...
                        on_progress("prompts", f"Fetching prompts for '{scenario_prefix}' from store...")
                        try:
                            ...
                            # Store may still contain {graph_name} / {scenario_prefix}
                            ...
                            on_progress("prompts", f"Could not fetch from store: {e}")
                    ...
                    for agent, default_prompt in defaults.items():
                        if agent not in prompts:
                            ...
                            on_progress("prompts", f"Using default prompt for {agent} (no stored prompt found)")
...
def _infra_nodes_only() -> list[dict]:
    ...
    return [
        ...
        {"id": "infra-cosmos-n", "label": "Cosmos DB (Metadata)", "type": "datasource",
