...
async def upload_graph(
    ...
    Loads vertices and edges into the graph backend. Returns SSE progress.
    ...
    async def work(progress: SSEProgress):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Guard: reject graph upload for Fabric scenarios (must be before schema read)
            graph_connector = (
            ...
            graph_name = graph_cfg.get("graph", f"{sc_name}-topology")
            ...
            # Use GraphBackend.ingest() for loading
            from backends import get_backend_for_graph

            backend = get_backend_for_graph(graph_name)
            ...
            progress.emit(
                "done",
                f"Graph loaded: {total_v} vertices, {total_e} edges → {graph_name}",
                100,
            )
