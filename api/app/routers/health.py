...
from app.gq_config import DATA_SOURCES, SCENARIO_NAME, DEFAULT_GRAPH
from app.backends import get_backend_for_graph
...
def _get_kql_singleton():
    ...
    if _kql_singleton is None:
        from app.backends.fabric_kql import FabricKQLBackend
...
def _get_gate_status() -> dict:
    ...
    try:
        from app.backends.fabric_throttle import get_fabric_gate
...
async def rediscover_fabric():
    ...
    from app.fabric_discovery import invalidate_cache, get_fabric_config, is_fabric_ready, is_kql_ready
