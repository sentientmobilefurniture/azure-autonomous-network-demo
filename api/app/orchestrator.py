...
async def run_orchestrator(alert_text: str) -> AsyncGenerator[dict, None]:
    ...
    class SSEEventHandler(AgentEventHandler):
        ...
        def _parse_structured_output(self, agent_name: str, raw_output: str) -> tuple:
            """Parse sub-agent output into (summary_text, visualizations_list).

            Returns:
                (response_text, list_of_visualization_dicts)
            """
            if not raw_output:
                return "", []

            # Try to extract ALL delimited query/results pairs (multi-query support)
            ...
            if status == "in_progress" and step.id not in self.step_starts:
                ...
                for tc in step.step_details.tool_calls:
                    ...
                    if tc_type == "connected_agent":
                        ...
                        if out:
                            response, visualizations = self._parse_structured_output(
                                agent_name, str(out),
                            )
                            if not response:
                                response = str(out)
...
# ---------------------------------------------------------------------------
# Session-aware orchestrator (persistent async sessions)
# ---------------------------------------------------------------------------
...
async def run_orchestrator_session(
    ...
    """Run the Foundry Orchestrator agent with session support.

    Like run_orchestrator() but with:
      1. cancel_event support — checked between steps/retry attempts
      2. existing_thread_id — reuse a Foundry thread for follow-up turns
      3. Emits thread_created event so SessionManager can track the thread
      4. No EVENT_TIMEOUT-based abort — session manager owns lifecycle

    See documentation/persistent.md §3.4 and §13.3.
    """
    from azure.ai.agents.models import AgentEventHandler
    ...
    # -- Handler (reuse the same SSEEventHandler from run_orchestrator) ------
    # We re-create a local class to keep the _put closure correct.
    ...
    class SSEEventHandler(AgentEventHandler):
        ...
        def _parse_structured_output(self, agent_name: str, raw_output: str) -> tuple:
            ...
            if query_blocks:
                ...
                # Structured delimiters present but all parses failed
                return summary, [{
                    "type": "documents",
                    "data": {"content": summary, "agent": agent_name},
                }]
