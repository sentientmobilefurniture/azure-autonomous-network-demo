...
6. **Disambiguated edge names (CRITICAL).** Some relationships have multiple target types. In Fabric GQL they are stored with a **suffixed name** — using the bare label returns zero results. You MUST use the suffixed form:
   - `monitors` → use `monitors_transportlink`, `monitors_corerouter`, or `monitors_amplifiersite`
   - `services` → use `services_corerouter` or `services_amplifiersite`
   - All other edges (`connects_to`, `routes_via`, `depends_on_mplspath`, `stationed_at`, etc.) use their plain name unchanged.
7. **Depot ↔ TransportLink requires multi-hop traversal.** There is no direct edge between Depot and TransportLink. To find which depots service a link, traverse via the CoreRouter (connects_to) or AmplifierSite (amplifies) intermediate nodes. See the "Depot ↔ TransportLink" section in core_schema.md for the two GQL query patterns.
...
---QUERY---
<second GQL query, verbatim — only if you made a second tool call>
---RESULTS---
<second query's COMPLETE JSON response, verbatim>
---ANALYSIS---
<your combined analysis and summary of ALL results for the orchestrator>
```

Include one `---QUERY---`/`---RESULTS---` pair per tool call, in chronological order.
If only one tool call was made, the format has a single QUERY/RESULTS block (same as above, minus the second pair).
The single `---ANALYSIS---` section always appears last, covering all queries.
